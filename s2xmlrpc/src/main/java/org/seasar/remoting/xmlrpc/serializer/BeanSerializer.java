...
/** 
 * JavaBeansをstructに変換するSerializerです。
 * A {@link TypeSerializer} for beans.
 */
public class BeanSerializer extends MapSerializer {
	...
	protected void writeData(ContentHandler pHandler, Object pData) throws SAXException {
		BeanDesc bd = BeanDescFactory.getBeanDesc(pData.getClass());
		for (int i = 0; i < bd.getPropertyDescSize(); i++) {
			PropertyDesc pd = bd.getPropertyDesc(i);
			writeEntry(pHandler, pd.getPropertyName(), pd.getValue(pData));
		}
	}
}